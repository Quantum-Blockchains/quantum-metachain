...
class LocalServerWrapper:
    ...
    def run(self):
        self.local_server.run(None, config.config_service.current_config.local_server_port, False)
...
def rotate_pre_shared_key(body):
    ...
    if is_local_peer:
        ...
        with open(config.config_service.current_config.abs_node_key_file_path()) as file:
            node_key = file.read()
            signature = sign(psk, node_key)

        with open(config.config_service.current_config.abs_psk_sig_file_path(), 'w') as file:
    ...
    create_psk_file(psk)
    sleep(config.config_service.current_config.key_rotation_time)

    node.node_service.current_node.restart()

    if os.path.exists(config.config_service.current_config.abs_psk_sig_file_path()):
        os.remove(config.config_service.current_config.abs_psk_sig_file_path())
