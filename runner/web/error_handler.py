...
from common.exceptions import PeerMisconfiguredError, PSKNotFoundError, PeerIsNotInHypercube, NoTargetReceived
...
def init_error_handlers(server: Flask):
    ...
    server.register_error_handler(NoTargetReceived, handle_no_target_received)
...
def handle_no_target_received(_e: NoTargetReceived):
    return make_response(json.dumps({"message": "No target received"}), 404)
