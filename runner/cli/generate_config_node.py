...
def create_node_dir(name_node, node_dir=None) -> str:
    if node_dir is not None:
        if path.exists(path.join(ROOT_DIR, node_dir)):
            return str(path.join(ROOT_DIR, node_dir))
        else:
            raise Exception(f"The folder {path.join(ROOT_DIR, node_dir)} does not exist")
    else:
        return str(path.join(CONFIG_DIR, name_node))


def copy_target(path_target, node_dir):
    if path.exists(path.join(ROOT_DIR, path_target)):
        shutil.copy2(str(path.join(ROOT_DIR, path_target)), node_dir)
    else:
        raise Exception(f"File {path.join(ROOT_DIR, path_target)} does not exist")
...
def generate_config_node(args):
    ...
    try:
        ...
        copy_target(args.qkd_target, node_dir)
        config = {
            "__type__": "Config",
