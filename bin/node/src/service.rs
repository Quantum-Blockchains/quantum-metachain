...
impl sc_executor::NativeExecutionDispatch for ExecutorDispatch {
	...
	fn dispatch(method: &str, data: &[u8]) -> Option<Vec<u8>> {
		qmc_runtime::api::dispatch(method, data)
	}

	fn native_version() -> sc_executor::NativeVersion {
		qmc_runtime::native_version()
	}
}
...
//     let rpc_port = rpc_address.port().to_le_bytes();
    let rpc_address = config.rpc_addr.unwrap();
    let rpc_port = rpc_address.port().to_le_bytes();
    if let Some(mut storage) = backend.offchain_storage() {
    ...
    if let Some(mut storage) = backend.offchain_storage() {
        storage.set(
            STORAGE_PREFIX,
            b"storage_number_of_block_for_restart_node",
            &bytes_number_of_block_for_restart_node,
        );
    };
